  ...
  public long largestSquareArea(int[][] bottomLeft, int[][] topRight) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = i + 1; j < n; j++) {
        ...
        int h = Math.min(topRight1[1], topRight2[1]) - Math.max(bottomLeft1[1], bottomLeft2[1]);
        int w = Math.min(topRight1[0], topRight2[0]) - Math.max(bottomLeft1[0], bottomLeft2[0]);
